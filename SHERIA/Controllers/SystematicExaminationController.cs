...
{
    public class SystematicExaminationController : Controller
    {
        ...
        public SystematicExaminationController(ILoggerManager logger, IWebHostEnvironment environment, DBHandler mydbhandler)
        {
        ...
        public class systematic_examination_record
        {
        ...
        public IActionResult Systematic()
        {
        ...
        [HttpPost]
        public ActionResult CreateSystematicExamination(systematic_examination_record record)
        {
            ...
            else
            {
                ...
                try
                {
                    SystematicExaminationModel existingrecord = dbhandler.GetSystematicExamination().Find(mymodel => mymodel.id == record.id)!;
                    if (existingrecord != null)
                    {
                        SystematicExaminationModel mymodel = new SystematicExaminationModel
                        {
                            id = existingrecord.id,
                            client_id = record.client_id,
                            systematic_examination_date = record.systematic_examination_date,
                            remarks = record.remarks,
                        };

                        if (dbhandler.UpdateSystematicExamination(mymodel))
                        {
                            CaptureAuditTrail("Updated systematic examination", "Updated systematic examination: " + mymodel.client_id);
                            ModelState.Clear();
                            response.error_code = "00";
                            response.error_desc = "Updated systematic examination successfully ";
                        }
                        else
                        {
                            response.error_code = "01";
                            response.error_desc = "Could not Updated systematic examination, kindly contact system admin ";
                        }
                    }
                    else
                    {
                        ...
                        if (dbhandler.AddSystematicExamination(mymodel))
                        {
                            CaptureAuditTrail("Created systematic examination", "Created systematic examination: " + mymodel.client_id);
                            ModelState.Clear();
                            response.error_code = "00";
                            response.error_desc = "systematic examination successfully created";
                        }
                        else
                        {
                            response.error_code = "01";
                            response.error_desc = "Could not Create sysytematic examination, kindly contact system admin";
                        }
                ...
                {
                    response.error_code = "01";
                    response.error_desc = "Could not Create systematic examination, kindly contact system admin";
                }
            }
        ...
        public ActionResult Delete(/*[FromBody] JObject jobject*/ int id, string module)
        {
            ...
            else
            {
                switch (module)
                {
                    case "systematic_examination_record":
                        SystematicExaminationModel systematic_examination_model = dbhandler.GetSystematicExamination().Find(mymodel => mymodel.id == id)!;
                        if (systematic_examination_model != null)
                        {
                            dbhandler.DeleteRecord(id, Convert.ToInt16(HttpContext.Session.GetString("userid")), module);
                            CaptureAuditTrail("Deleted systematic examination record", "Deleted systematic examination record: " + systematic_examination_model.client_id);
                        }
                        break;
